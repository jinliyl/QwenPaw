        ...
        for block in output:
            try:
                if not isinstance(block, dict) or "type" not in block:
                    logger.warning(f"Invalid block: {block}, expected a dict with 'type' key, skipped.")
                    continue
                ...
                if block_type == "text":
                    ...
                    logger.warning(f"Unsupported block type '{block_type}' in tool result, skipped.")

            except Exception as e:
                logger.warning(f"Failed to process block {block}: {e}, skipped.")

...
记忆管理遵循三阶段自动化工作流：**白天积累 → 晚上整合 → 次日检索**。推荐组合使用，让记忆从原始记录逐步结晶为高价值知识。

### 积累阶段 - Auto-Memory

Auto-Memory 是自动记忆总结能力，在特定时机自动将对话中的重要信息持久化保存到 `memory/YYYY-MM-DD.md` 中。
...
- **输出**：按日期组织的原始日志文件，同一天多次总结会智能合并，避免重复
- **特点**：保留所有细节，不做筛选或优化

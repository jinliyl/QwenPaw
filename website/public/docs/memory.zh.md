...
| 可复用经验           | `MEMORY.md`            | `write` / `edit` 工具 | 梦境优化从日常记录中提炼的核心知识         |

---

## 🌙 梦境优化机制

**梦境优化**是 CoPaw 的自动记忆整理功能，会在预设时间（默认每晚23:00）运行。该功能自动从日常记录中提炼**核心业务决策、确认的用户偏好和高价值可复用经验**，通过四大原则优化 MEMORY.md：
...
由 `dream_cron` 配置项控制（默认 `"0 23 * * *"`），设为空字符串可禁用。

...
- **功能**：自动将日常交互和笔记作为原始记录写入 `memory/` 目录
- **输入**：用户对话、Agent 工具调用结果、用户明确的"记住这个"指令
- **输出**：按日期组织的原始日志文件 `memory/YYYY-MM-DD.md`
- **特点**：保留所有细节，不做筛选或优化

### 整合阶段（晚上） - Auto-Dream

- **核心理念**：QwenPaw 的智能记忆整合系统，在安静时段自动优化 MEMORY.md
- **触发时间**：默认每晚 11 点（Cron 表达式 `"0 23 * * *"`）
- **输入源**：从 `memory/` 目录读取当日及历史日志
- **五大优化原则**：
  - **去除噪音**：删除临时细节、bug修复记录和一次性任务
...
### 检索阶段（次日） - Auto-Memory-Search

- **功能**：检索 Auto-Dream 优化后的 MEMORY.md（更精准）
- **优势**：基于结晶化知识提供高质量搜索结果
- **效果**：确保检索内容的精准性和高价值
